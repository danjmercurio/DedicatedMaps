...
God.watch do |w| 
  ...
  w.start = "/bin/bash -c 'cd #{APPLICATION_ROOT}; /usr/bin/env RAILS_ENV=#{RAILS_ENV} #{APPLICATION_ROOT}/script/delayed_job start > /tmp/delay_job.out'"
  w.stop = "/bin/bash -c 'cd #{APPLICATION_ROOT}; /usr/bin/env RAILS_ENV=#{RAILS_ENV} #{APPLICATION_ROOT}/script/delayed_job stop'"

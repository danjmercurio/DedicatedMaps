...
class StagingArea < ActiveRecord::Base
  ...
  def self.parse_uploads(params)
    ...
    begin
      ...
      # Get the contents and construct Nokogiri parser instance
      ...
      if locationsNoko.search($LOCATIONS_XML_IDENTIFIER).count <= 0
        message += 'XML Parser found no locations.'
      ...
      locationsNoko = xslt.transform(locationsNoko)
        ...
        # Optionally create a StoredText
    ...
    if params.has_key? :map_asset_types
      begin
        ...
        if assetTypesNoko.search($ASSET_TYPES_XML_IDENTIFIER).count <= 0
          message += 'XML Parser found no map asset types'
          raise Nokogiri::XML::SyntaxError
        end
        ...
        message += e.to_s + " in " + assetTypesFile.original_filename
        return message
      rescue Exception => e
        message += e.to_s + " in " + assetTypesFile.original_filename
        return message
    ...
    if params.has_key? :map_assets
      begin
        ...
        if assetsNoko.search($ASSETS_XML_IDENTIFIER).count <= 0
          message += 'XML Parser found no map assets. '
          raise Nokogiri::XML::SyntaxError
        end
        ...
        message += e.to_s + " in " + assetsFile.original_filename
        return message
      rescue Exception => e
        message += e.to_s + " in " + assetsFile.original_filename
        return message
    ...
    return 'Error: Attempted to insert locations but routine to do so returned no location IDs' unless location_ids.count > 0

    if assetTypesNoko
      type_ids = insert_types(assetTypesNoko, params[:company_id])
    end

    if assetsNoko && assetTypesNoko
